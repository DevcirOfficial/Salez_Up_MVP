...
export default function SeniorOpsManagerPage() {
  const navigate = useNavigate();
  const [opsManagers, setOpsManagers] = useState([]);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [selectedOpsManager, setSelectedOpsManager] = useState(null);
  ...
  const fetchOpsManagers = async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch Senior Ops managers.");
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success("Senior Ops manager deleted successfully.");
      } else {
        toast.error("Failed to delete Senior Ops manager.");
      }
    } catch (error) {
  ...
  const confirmDelete = (opsManager) => {
    setSelectedOpsManager(opsManager);
    setIsDeleteModalOpen(true);
  };

  const handleEdit = (opsManager) => {
    setSelectedOpsManager(opsManager);
    setFormData({
      manager_name: opsManager.manager_name,
      manager_commision: opsManager.manager_commision,
      manager_email: opsManager.manager_email,
      manager_password: decryptPassword(opsManager.manager_password),
      head_of_sales_id: opsManager.head_of_sales
        ? opsManager.head_of_sales.id
        : null,
    });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchOpsManagers();
        toast.success("Senior Ops manager updated successfully.");
      } else {
        toast.error("Failed to update Senior Ops manager.");
      }
    } catch (error) {
      ...
      <div className="flex justify-between">
        <div>
          <h2 className="text-3xl font-bold text-left">
            Senior Operations Manager Dashboard
          </h2>
          <h3 className="mt-4 mb-4 text-lg font-semibold text-left">
            Senior Operations Manager List
          </h3>
        </div>
        <button
          onClick={() => {
            navigate("/SeniorOpsManger_SignUp");
          }}
        ...
        >
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="text-xl">Register SOM</span>
          </div>
        </button>
      </div>
            ...
            >
              <div className="flex items-center justify-center mb-6">
                <div className="rounded-full">
                  <img src={opsManager.manager_image_path} alt="" />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-md text-gray-700">
                  <FontAwesomeIcon
                    ...
                    className="mr-2 text-[#202938d2]"
                  />
                  {opsManager.head_of_sales
                    ? opsManager.head_of_sales.manager_name
                    : "Not Assigned"}
                  <b>(Head of sales)</b>
                </h2>
              </div>
              <div className="mt-2 space-y-6">
                ...
                <p className="mt-2 text-gray-600">
                  <FontAwesomeIcon
                    ...
                    className="mr-2 text-[#202938d2]"
                  />
                  {opsManager.manager_commision || 0}%
                </p>
                <p className="mt-2 text-gray-600">
                  <FontAwesomeIcon
                    icon={faKey}
                    className="mr-2 text-[#202938d2]"
                  />
                  {decryptPassword(opsManager.manager_password)}
                </p>
              </div>
              <div className="flex justify-between mt-6 space-x-2">
                <button
                  onClick={() => handleEdit(opsManager)}
                ...
                <button
                  onClick={() => confirmDelete(opsManager)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 mx-4 bg-white rounded-lg">
            ...
            <p className="mb-6">
              ...
              <b>{selectedOpsManager.manager_name}</b> (Senior Ops Manager)?
            </p>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 mx-4 bg-white rounded-lg">
            ...
            <h2 className="mb-4 text-xl font-bold">Edit Senior Ops Manager</h2>
