...
const Senior_Ops_manager_SignUp = () => {
  ...
  const [ops_manager_name, setOpsManagerName] = useState('');
  ...
  const handleSignup = async (e) => {
    ...
    if (!ops_manager_name || !email || !password) {
      fillAllCredentials();
      return;
    }
  ...
  const handleOtpSubmit = async (e) => {
    ...
    const ops_manager_secret_id = generatePassword();
      ...
      if (otpResponse.ok) {
        ...
        try {
          ...
          const formData = {
            ...
            commission: 10,
            ops_manager_secret_id: parseInt(ops_manager_secret_id)
          };
          ...
          const response = await axios.post("https://crmapi.devcir.co/api/senior-ops-managers", formData);
          console.log("Registration successful:", response.data);

          try {
            const response = await fetch('https://crmapi.devcir.co/api/send-link2', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
